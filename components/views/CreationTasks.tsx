...
const MOCK_SHOTS_EP1: Shot[] = [
  {
    ...
    prompt: '近景，一只麻雀在树枝上叽叽喳喳。背景是秘境院子，模糊的蓝天和摇曳的树影，日漫赛璐珞上色风格。',
  ...
  },
  {
    ...
    prompt: 'Lu Yin holding a whetstone, muscles tense, grinding a broken sword, sparks flying, intense eyes, macro shot, anime style.',
  ...
  },
  {
    ...
    prompt: 'Lin Youwei trembling, fearful expression, dark background, cinematic anime style.',
  ...
  },
  {
    ...
    prompt: 'Three demons appearing, giant axe, chain scythe, sparks on stone floor, ominous atmosphere, anime style.',
    dialogue: '大鬼: 终于找到你了...',
  ...
  },
  {
    ...
    scene: '大鬼发起攻击，陆隐躲避。',
    prompt: 'Giant demon attacking with dual axes, dynamic action shot, motion blur, Lu Yin dodging.',
    dialogue: '(巨大的撞击声)',
...
const MOCK_SHOTS_EP2: Shot[] = [
  {
    ...
    prompt: 'Master standing on mountain peak, back view, clouds, mysterious atmosphere.',
    dialogue: '师尊: 你来了。',
    camera: 'Wide Shot (全景)',
    assets: [],
    status: 'pending',
...
const CreationTasks: React.FC = () => {
  ...
  const [shots, setShots] = useState<Shot[]>(MOCK_SHOTS_EP1);
  const [selectedShotId, setSelectedShotId] = useState<string>(MOCK_SHOTS_EP1[0].id);
  ...
  const handleEpisodeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (ep === EPISODES[0]) {
      setShots(MOCK_SHOTS_EP1);
      setSelectedShotId(MOCK_SHOTS_EP1[0].id);
    } else {
      setShots(MOCK_SHOTS_EP2);
      setSelectedShotId(MOCK_SHOTS_EP2[0].id);
    }
  };
  ...
  return (
    <div className="h-full flex bg-[#0f172a] text-slate-200 overflow-hidden relative">
          ...
          {shots.map(shot => (
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <span className="font-medium text-sm line-clamp-1" title={shot.name}>{shot.name}</span>
                <span className="text-xs bg-slate-950 px-1.5 py-0.5 rounded text-slate-500">{shot.duration}s</span>
              </div>
              <p className="text-xs opacity-70 line-clamp-2 mb-2">{shot.description}</p>
      ...
      <div className="w-[420px] bg-slate-900 border-r border-slate-800 flex flex-col shrink-0 z-10">
            ...
            <div className="space-y-6 animate-fadeIn">
               ...
               <div className="space-y-4">
                  <div className="space-y-1">
                   ...
                   <div className="space-y-1">
                     <label className="text-xs font-bold text-blue-400 uppercase tracking-wider">Prompt (详细)</label>
                     <textarea
                       value={selectedShot.prompt}
                       onChange={(e) => handleScriptUpdate('prompt', e.target.value)}
                       className="w-full text-sm text-slate-300 bg-slate-800/30 p-3 rounded border border-slate-700/50 leading-relaxed focus:border-blue-500/50 focus:bg-slate-800 outline-none resize-none min-h-[120px]"
                     />
                  </div>
                   <div className="space-y-1">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0b1120] relative">
        ...
        <div className="h-14 border-b border-slate-800 flex items-center px-6 gap-6 bg-slate-900 justify-end">
           ...
           <button 
             ...
             className={`text-sm font-medium h-full px-2 border-b-2 transition-colors ${previewMode === 'edit' ? 'text-white border-primary' : 'text-slate-500 border-transparent hover:text-white'}`}
           >
             视频编辑
           </button>
        </div>
