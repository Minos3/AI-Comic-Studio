              ...
              <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl flex flex-col group hover:border-slate-700">
                ...
                <div className="p-4 flex-1 flex flex-col gap-2">
                  ...
                  <div className="flex gap-2 mt-auto">
                    <button onClick={() => handleGenerate(c)} className="text-[11px] font-bold px-2.5 py-1.5 bg-primary text-white rounded-lg hover:bg-indigo-600 transition-colors">AI 生成</button>
                    <button onClick={() => { setUploadingFor(c.id); fileRef.current?.click(); }} className="text-[11px] font-bold px-2.5 py-1.5 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-800 transition-colors">上传图片</button>
                    <button onClick={() => handleDelete(c.id)} className="text-[11px] text-red-400 hover:text-red-300 ml-auto">删除</button>
                  </div>
                </div>
              </div>
            ))}
