...
const SceneManager: React.FC<{ projectId?: number }> = ({ projectId }) => {
  ...
  const handleCreateScene = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/projects/${projectId}/scenes`, { name: newName, description: newDesc, styleKeywords: newKeywords });
      setShowCreate(false); setNewName(''); setNewDesc(''); setNewKeywords('');
      loadScenes();
    } catch { alert('创建失败'); }
  };
    ...
    return (
      <div className="h-full flex items-center justify-center bg-[#05080f]">
        <div className="text-center">
          ...
          <p className="text-slate-500 text-sm mb-4">暂无场景数据</p>
          <button
            onClick={() => { setNewName(''); setNewDesc(''); setNewKeywords(''); setShowCreate(true); }}
      ...
      <div className="w-[300px] shrink-0 border-r border-slate-800/50 flex flex-col bg-[#0b0f1a] z-20">
        <div className="p-4 border-b border-slate-800/50 bg-slate-900/40">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => { setNewName(''); setNewDesc(''); setNewKeywords(''); setShowCreate(true); }}
